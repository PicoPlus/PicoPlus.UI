...
public class AuthService
{
    ...
    /// <summary>Find a contact by national code. Returns null if not found.</summary>
    public async Task<Contact?> FindContactByNationalCodeAsync(string nationalCode, CancellationToken ct = default)
    {
        _logger.LogInformation("Looking up contact for national code: {NationalCode}", nationalCode);
        return await _contactRepo.FindByNationalCodeAsync(nationalCode);
    }

    /// <summary>Auto-enrich missing fields on an existing contact via Zohal.</summary>
    public async Task<Contact> EnrichContactAsync(Contact contact, CancellationToken ct = default)
    {
        _logger.LogInformation("Enriching contact: {ContactId}", contact.Id);
        return await _contactRepo.UpdateMissingFieldsAsync(contact, ct);
    }
